...
//		}
		...
		String sql = "select a.id,a.title, a.author,a.views,a.status, a.time, concat('/cms?type=project','&id=',CAST(a.id as char),'&time=',DATE_FORMAT(a.time,'%Y%m%d'))  url, c.name as cname ,'article' model, a.stick from "+config.getPrefix()+"project as a "
