 ...
 * @version 1.0
 */
public class TemplateResourceLoad {
	 ...
	 * @throws IOException
	 */
	public TemplateResourceLoad(Resource resource) throws IOException {
		loadResource(resource);
		this.readModified = lastModified();// 读取时间
	}

	private void loadResource(Resource resource) throws IOException {
		resources.add(resource);// 加入模板文件维护集合


		InputStream   __fis = resource.getInputStream();//文件字节流
		...
		while ((temp = __br.readLine()) != null) {
			Resource childFile = compre(resource, temp);
			if(childFile != null){
				loadResource(childFile);// 递归调用加载引用模板
			}else{
	 ...
	 * @param linedata
	 */
	private Resource compre(Resource currentFile, String linedata) throws IOException {
		Matcher matcher = includeTagPattern.matcher(linedata);
		while(matcher.find()){// 找到两个
    		String text = matcher.group(1);
			return  currentFile.createRelative(text);
		}
		return null; 
	}
