 ...
 */
public class ThematicModelImpl extends ContentModel {
    ...
    public ThematicModelImpl() {
        ...
        config.put("name", "专题模型");
        config.put("author", "marker");
        config.put("version", "0.1");
        config.put("type", "thematic");
        config.put("template", "thematic.html");
...
//		}
        ...
        sql.append("select A.*,C.name as cname,concat('type=thematic&id=',CAST(A.id as char),'&time=',DATE_FORMAT(A.time,'%Y%m%d')) as url from ")
                .append(prefix).append("thematic").append(SQL.QUERY_FOR_ALIAS)
    ...
    public StringBuilder doWebFront(String tableName, WebDataSource WebDataSource) {
        ...
        StringBuilder sql = new StringBuilder("select  M.*,C.name cname, concat('/cms?','type=thematic','&id=',CAST(M.id as char),'&time=',DATE_FORMAT(M.time,'%Y%m%d')) url from" +
                " "+prefix+"channel C "
                + "right join "+prefix+"thematic M on M.cid = C.id");
